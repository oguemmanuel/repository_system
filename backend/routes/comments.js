...
router.post("/", isAuthenticated, async (req, res) => {
  try {
    ...
    const [resources] = await db.query("SELECT * FROM resources WHERE id = ?", [
      resourceId,
    ]);
    ...
    const [comments] = await db.query(
      `SELECT c.*, u.fullName, u.role FROM comments c JOIN users u ON c.userId = u.id WHERE c.id = ?`,
      [result.insertId],
    );
...
router.get("/resource/:resourceId", async (req, res) => {
  try {
    ...
    const [comments] = await db.query(
      `SELECT c.*, u.fullName, u.role FROM comments c JOIN users u ON c.userId = u.id WHERE c.resourceId = ? ORDER BY c.createdAt DESC`,
      [resourceId],
    );
  ...
  try {
    ...
    if (comment.userId !== userId && userRole !== "admin") {
      const [resources] = await db.query(
        "SELECT uploadedBy FROM resources WHERE id = ?",
        [comment.resourceId],
      );
